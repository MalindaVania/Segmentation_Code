...
def CrossEntropy2d(input, target, weight=None, size_average=False):
    ...
    if size_average:
        ...
        #loss /= target_mask.sum()
        loss /= target_mask.sum().float()
...
class CycleSEGModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_seg = networks.define_G(opt.input_nc_seg, opt.output_nc_seg,
                                        opt.ngf, opt.which_model_netSeg, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def backward_G(self):
        ...
        if self.opt.seg_norm == 'DiceNorm':
            ...
            weight = torch.from_numpy(arr).float().cuda()
            #weight = torch.from_numpy(arr).cuda().long()
            self.loss_seg = cross_entropy2d(self.seg_fake_B.float(), self.real_Seg_one.float(), weight=weight.cuda())
    ...
    def optimize_parameters(self):
        ...
        # D_A
        self.set_requires_grad([self.netD_A, self.netD_B], True) # 070819
    ...
    def get_current_errors(self):
        D_A = self.loss_D_A.data #D_A = self.loss_D_A.data[0]
        G_A = self.loss_G_A.data #G_A = self.loss_G_A.data[0]
        Cyc_A = self.loss_cycle_A.data #Cyc_A = self.loss_cycle_A.data[0]
        D_B = self.loss_D_B.data #D_B = self.loss_D_B.data[0]
        G_B = self.loss_G_B.data #G_B = self.loss_G_B.data[0]
        Cyc_B = self.loss_cycle_B.data #Cyc_B = self.loss_cycle_B.data[0]
        Seg_B = self.loss_seg.data #Seg_B = self.loss_seg.data[0]
        if self.opt.identity > 0.0:
            idt_A = self.loss_idt_A.data[0] #self.loss_idt_A.data[0]
    ...
    def save(self, label):
        #writer = SummaryWriter('/home/mvania/Seg-Net/Checkpoints/train2/')
