...
def cross_entropy2d(input, target, weight=None, size_average=True):
    ...
    loss = F.nll_loss(log_p, target.long(), weight=weight.float(), size_average=False)
    if size_average:
        #loss /= mask.data.sum()
...
class CycleSEGModel(BaseModel):
    ...
    def backward_G(self):
        ...
        if self.opt.seg_norm == 'DiceNorm':
            ...
            self.loss_seg = cross_entropy2d(self.seg_fake_B.cuda().float(), self.real_Seg_one.float(), weight=weight)
